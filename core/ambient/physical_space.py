...
class PhysicalSpace:
    ...
    def can_place(self, token, x, y, shape, ignore=None, blocks=True):
        ignore = ignore or ()
        for cell in self._absolute_cells(x, y, shape):
    ...
    def place(self, layer, token, x, y, shape=None, blocks=False):
        ...
        if not self.can_place(token, x, y, shape, ignore=key, blocks=blocks):
            raise ValueError("shape sem espaço físico")
        cells = self._absolute_cells(x, y, shape)
        self._records[key] = (x, y, shape, bool(blocks))
        for cell in cells:
    ...
    def remove(self, layer, token):
        ...
        for cell in self._absolute_cells(x, y, shape):
            ...
            if blocks and self._blocking.get(cell) == key:
                del self._blocking[cell]

    def move(self, layer, token, x, y):
        key = self._key(layer, token)
        old_x, old_y, shape, blocks = self._records[key]
        if not self.can_place(token, x, y, shape, ignore=key, blocks=blocks):
            return False
        self.remove(layer, token)
        self.place(layer, token, x, y, shape, blocks)
        return True

    def cells_for(self, layer, token):
        x, y, shape, _blocks = self._records[self._key(layer, token)]
        return self._absolute_cells(x, y, shape)

    def local_cells_for(self, layer, token):
        _x, _y, shape, _blocks = self._records[self._key(layer, token)]
        return shape.cells
    ...
    def _same_anchor(self, left, right):
        ...
        return bool(left_record and right_record and left_record[:2] == right_record[:2])
    ...
    def visible_parts(self, viewer_x, viewer_y, layer, token, ignore=()):
        ...
        record_x, record_y, shape, _blocks = self._records[key]
        visible = []
        for local, cell in zip(shape.cells, cells):
